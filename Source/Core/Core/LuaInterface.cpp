...
DEFINE_LUA_FUNCTION(memory_readfloat, "address")
{
/*	unsigned long address = luaL_checkinteger(L,1);
	unsigned long value = Memory::Read_U32(address);
	lua_settop(L,0);
	float corrected = reinterpret_cast<float> (value);
	char buffer[9000];
...
DEFINE_LUA_FUNCTION(state_load, "location[,option]")
{
	...
	switch(type)
	{
		...
		default:
		{
			...
			State::Load((int)luaL_checkinteger(L,1));
...
{
	lua_pushinteger(L, (lua_Integer)Movie::g_currentLagCount);
	return 1;
}
#if 0
DEFINE_LUA_FUNCTION(emulua_lagged, "")
{
	lua_pushboolean(L, !Movie::g_bPolled);
	return 1;
}
#endif
...
{
	lua_pushinteger(L, (Movie::IsPlayingInput() || Movie::IsRecordingInput()) ? (lua_Integer)Movie::g_totalFrames : 0);
	return 1;
}
DEFINE_LUA_FUNCTION(movie_isactive, "")
{
	lua_pushboolean(L, (Movie::IsPlayingInput() || Movie::IsRecordingInput()));
	return 1;
}
DEFINE_LUA_FUNCTION(movie_rerecordcount, "")
{
	//lua_pushinteger(L, (Movie::IsPlayingInput() || Movie::IsRecordingInput()) ? (lua_Integer)Movie::g_rerecords : 0);
	return 1;
}
#if 0
DEFINE_LUA_FUNCTION(movie_setrerecordcount, "")
{
	Movie::g_rerecords = (int)luaL_checkinteger(L, 1);
	return 0;
}
#endif
...
{
	lua_pushboolean(L, Movie::IsRecordingInput());
	return 1;
}
DEFINE_LUA_FUNCTION(movie_isplaying, "")
{
	lua_pushboolean(L, Movie::IsPlayingInput());
	return 1;
}
#if 0 
DEFINE_LUA_FUNCTION(movie_getmode, "")
{
	switch(Movie::g_playMode)
	{
	case Movie::MODE_PLAYING:
		lua_pushstring(L, "playback");
		break;
	case Movie::MODE_RECORDING:
		lua_pushstring(L, "record");
		break;
	case Movie::MODE_NONE:
...
{
//	lua_pushstring(L, Movie::g_recordFile.c_str());
	return 0;
}
...
DEFINE_LUA_FUNCTION(movie_replay, "")
{
	...
	{
		std::string filename = Movie::g_recordFile;
		Movie::EndPlayInput(false);
		Movie::PlayInput(filename.c_str());
	} 
	else
...
static const struct luaL_reg memorylib [] =
{
	...
	{"readfloat", memory_readfloat}, //hope this works
